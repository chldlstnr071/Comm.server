<?php
	...
	for($i=0; $row = mysqli_fetch_array($result); $i++){
		...
		array_push($r, array('fire1'=>$row[1], 'doppler1'=>$row[2], 'time'=>$row[3]));

		if($i==0){

			array_push($res, array('fire1'=>$row[1], 'doppler1'=>$row[2], 'time'=>$row[3]));
		}

		else if(($i!=0)&&(($r[$a]['fire1']!=$res[$cnt]['fire1'])||($r[$a]['doppler1']!=$res[$cnt]['doppler1']))){
